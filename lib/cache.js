...
CacheItem.prototype.lock = function(name, callback){
  ...
  _lock.lock(function(){
    ...
    var timer = setTimeout(function(){
      console.error('CacheItem lock release failed: ', id, name);
      console.error(e.stack);
      failed = true;
    }, 5000);
    this.unlock = function(dirty){
      clearTimeout(timer);
      if(failed){
        console.error('Eventual lock cleanup: ', id, name);
      }
...
Cache.prototype.lock = function(name, callback){
  ...
  _lock.lock(function(){
    ...
    var timer = setTimeout(function(){
      failed = true;
      console.error('Cache lock release failed: ', name);
      console.error(e.stack);
    }, 5000);
...
Cache.prototype.flushChanges = function(){
  ...
  var processItem = function(item, next){
    ...
    item.lock('processItem-'+(hasId?'update':'insert'), function(){
      if(hasId){
